...
for filename in filenames:
	with open(f'{INPUT}/{filename}', encoding='utf8') as input:
			...
			with open(f'{OUTPUT}/{namespace}.bf', 'wt') as output:
				...
				if len(constants) > 0:
					...
					for constant in constants:
						...
						if type == 'Guid':
						...
						elif type == 'PROPERTYKEY' or type == 'DEVPROPKEY':
							fmtid = guid_literal(value['Fmtid'])
							pid = value['Pid']
							output.write(f'{indent}public const {type} {name} = .({fmtid}, {pid});\n')
						elif type == 'String':
							if value is None: value = '' # For some reason this happens for Globalization.LOCALE_NAME_INVARIANT.
							...
							output.write(f'{indent}public const {type} {name} = (PSTR)(void*){value};\n')
						elif type == 'PWSTR':
							output.write(f'{indent}public const {type} {name} = (PWSTR)(void*){value};\n')
						elif type == 'SOCKET': # SOCKET INVALID_SOCKET = -1 gives error on implicit conversion from int to uint.
							output.write(f'{indent}public const {type} {name} = (SOCKET){value};\n')
						else:
							output.write(f'{indent}public const {type} {name} = {value};\n')
				...
				if len(enums) > 0:
					...
					for type in enums:
						...
						if name == 'D2D1_2DAFFINETRANSFORM_INTERPOLATION_MODE':
							x = 123
					...
					for type in structs_and_unions:
						...
						def process_type(type):
							...
							if len(fields) == 0:
								...
								if name == 'PROPERTYKEY' or name == 'DEVPROPKEY':
